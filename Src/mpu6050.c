...
int8_t MPU6050_Read_Accel(void)
{
	...
	int8_t status = Sensor_I2C2_Read(MPU6050_READ_ADDR, ACCEL_XOUT_H, Read_Buf, 6); 
	
	x = (int16_t)(Read_Buf[0] << 8 | Read_Buf[1]);
	y = (int16_t)(Read_Buf[2] << 8 | Read_Buf[3]);
	z = (int16_t)(Read_Buf[4] << 8 | Read_Buf[5]);
	
	Mpu6050_Data.Accel_X = x / 16384.0f;
	Mpu6050_Data.Accel_Y = y / 16384.0f;
	Mpu6050_Data.Accel_Z = z / 16384.0f;
	return status;
}
...
MPU6050DATATYPE MPU6050_Get_Data(void){
	return Mpu6050_Data;
}
